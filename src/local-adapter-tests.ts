...
declare
  writer_oid oid;
begin
  select oid into writer_oid from pg_roles where rolname = 'atlas_session_writer' and not rolcanlogin;
...
begin
  if (select array_agg(p.proname::text order by p.proname, p.oid)
     ...
     is distinct from array[
       'session_activate', 'session_confirm_closed', 'session_create',
  ...
  if exists (
    ...
    where n.nspname in ('app', 'public') and c.relkind in ('r', 'p', 'v', 'm', 'f')
      and has_table_privilege('${ATLAS_WRITER_LOGIN}', c.oid,
