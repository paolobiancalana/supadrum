...
and registered path. A failed or mismatched setup blocks the test job. The
new operation does not run arbitrary SQL; the existing digest-pinned
`sql.execute` remains the only fixture-loading path.
